...
CREATE TABLE IF NOT EXISTS `postal_office`.`operations_payment_methods` (
  ...
  PRIMARY KEY (`payment_method_id`, `operation_id`),
...
begin
    ...
    SET @count = NEW.number_sold;
    set @good_for_sale_id = NEW.good_for_sale_id;
    update goods_for_sale set goods_for_sale.good_amount = (goods_for_sale.good_amount - @count) where goods_for_sale.good_for_sale_id LIKE @good_for_sale_id;
    update operations set operations.total_price = (operations.total_price + @perem) where operations.operation_id LIKE @oper_id;
end$$

USE `postal_office`$$
CREATE DEFINER = CURRENT_USER TRIGGER `postal_office`.`sold_goods_BEFORE_DELETE` BEFORE DELETE ON `sold_goods` FOR EACH ROW
BEGIN
	SET @perem = OLD.sold_goods_price;
	SET @oper_id = OLD.operation_id;
    SET @count = OLD.number_sold;
    set @good_for_sale_id = OLD.good_for_sale_id;
    update goods_for_sale set goods_for_sale.good_amount = (goods_for_sale.good_amount + @count) where goods_for_sale.good_for_sale_id LIKE @good_for_sale_id;
    update operations set operations.total_price = (operations.total_price - @perem) where operations.operation_id LIKE @oper_id;
END$$

USE `postal_office`$$
CREATE DEFINER = CURRENT_USER TRIGGER `postal_office`.`delivery_goods_BEFORE_DELETE` BEFORE DELETE ON `delivery_goods` FOR EACH ROW
BEGIN
	SET @perem = OLD.delivery_good_price;
	SET @oper_id = OLD.operation_id;
    update operations set operations.total_price = (operations.total_price - @perem) where operations.operation_id LIKE @oper_id;
END$$

USE `postal_office`$$
CREATE DEFINER = CURRENT_USER TRIGGER `postal_office`.`delivery_goods_AFTER_INSERT` AFTER INSERT ON `delivery_goods` FOR EACH ROW
BEGIN
	SET @perem = NEW.delivery_good_price;
	SET @oper_id = NEW.operation_id;
    update operations set operations.total_price = (operations.total_price + @perem) where operations.operation_id LIKE @oper_id;
END$$

USE `postal_office`$$
CREATE DEFINER = CURRENT_USER TRIGGER `postal_office`.`completed_payments_AFTER_INSERT` AFTER INSERT ON `completed_payments` FOR EACH ROW
BEGIN
	SET @perem = NEW.payment_price;
	SET @oper_id = NEW.operation_id;
    update operations set operations.total_price = (operations.total_price + @perem) where operations.operation_id LIKE @oper_id;
END$$

USE `postal_office`$$
CREATE DEFINER = CURRENT_USER TRIGGER `postal_office`.`completed_payments_BEFORE_DELETE` BEFORE DELETE ON `completed_payments` FOR EACH ROW
BEGIN
	SET @perem = OLD.payment_price;
	SET @oper_id = OLD.operation_id;
    update operations set operations.total_price = (operations.total_price - @perem) where operations.operation_id LIKE @oper_id;
END$$
